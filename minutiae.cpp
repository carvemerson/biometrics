...
bool MaskMatch(int x, int y, Mat &img, Mat &mask)
{
    int k, l, i, j;
    bool res = true;
    ...
    for(i = x-1, k = 0;k < mask.rows && res;i++, k++)
    {
        ...
        {
            res = res && (img.at<uchar>(i, j) == mask.at<uchar>(k, l));
        }
    }
...
bool EndLine(int x, int y, Mat &img)
{
    int res = 0;

    res += (int)(img.at<uchar>(x-1, y-1) == 255);
    res += (int)(img.at<uchar>(x-1, y) == 255);
    res += (int)(img.at<uchar>(x-1, y+1) == 255);
    res += (int)(img.at<uchar>(x, y-1) == 255);
    res += (int)(img.at<uchar>(x, y+1) == 255);
    res += (int)(img.at<uchar>(x+1, y-1) == 255);
    res += (int)(img.at<uchar>(x+1, y) == 255);
    res += (int)(img.at<uchar>(x+1, y+1) == 255);
...
bool Bifurcation(int x, int y, Mat &img)
{
    int res = 0;
